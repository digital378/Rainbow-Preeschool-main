...
- **15-keyword commercial recovery (April 24, 2026)**: 5 destination pages (`/play-school-near-me`, `/best-preschool-near-me-in-thane`, `/playgroup`, `/nursery`, `/kindergarten`) carry the 15 priority commercial keywords. Each page emits FAQPage JSON-LD; the three programme pages additionally emit Organization JSON-LD for schema parity with the locality pages. `/play-school-near-me` and `/best-preschool-near-me-in-thane` each carry ≥1,300 words of unique commercial content. The homepage body content links to all 5 destinations via the new `contentSections[].links` field rendered by `server/bot-ssr.ts`. 21 ghost-slug variants (e.g. `/playgroup-near-me`, `/best-kindergarten-in-thane`, `/playschool-near-me`, `/preschool-near-me`, plus all trailing-slash pairs) 301 to the correct canonical in `server/redirects.ts`. Smoke test: `tsx scripts/check-keyword-targets.ts` — exits non-zero if FAQPage JSON-LD, programme-page Organization schema, deep-content word count, homepage anchors, or any ghost-slug 301 regresses. Documented in `docs/seo-seasonal-refresh-playbook.md`.
