...
-   **Pre-push Safety Net**: A pre-push hook (`.githooks/pre-push`) re-runs `npm run check` (plus the no-pink, title-cannibalisation, and description-length guards) before `git push`, so commits that slipped past pre-commit (e.g. via `--no-verify`) still can't reach the remote. It is auto-installed by `bash scripts/install-hooks.sh` and skips re-running if pre-commit just succeeded on the same tree. Bypass once with `git push --no-verify` or `SKIP_PREPUSH=1 git push ...`.