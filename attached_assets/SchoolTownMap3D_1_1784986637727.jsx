...
function Scene({ setActive, activeId, hoveredId, setHovered, reduced, isMobile }) {
  ...
  return (
    <>
      {!isMobile && <SoftShadows size={26} samples={10} />}
      <ambientLight intensity={isMobile ? 0.95 : 0.55} />
      <directionalLight position={[6, 11, 5]} intensity={isMobile ? 1.2 : 1.5} castShadow={!isMobile} shadow-mapSize={[1024, 1024]} shadow-bias={-0.0002} />
      ...
      {(isMobile ? TREES.slice(0, 3) : TREES).map((t, i) => <Tree key={i} position={t.p} s={t.s} />)}
      {BRANCHES.map((b) => (
      ...
      <ContactShadows position={[0, 0.005, 0]} opacity={0.35} scale={16} blur={2.4} far={4} resolution={isMobile ? 256 : 512} />
      {!isMobile && <ContactShadows position={[0, -2.4, 0]} opacity={0.25} scale={24} blur={4} far={6} color="#3a5a2a" />}

      {/* Mobile: no manual rotate (so the page scrolls normally) — gentle auto-rotate instead */}
      <OrbitControls enablePan={false} enableZoom={!isMobile} enableRotate={!isMobile}
        autoRotate={isMobile && !reduced} autoRotateSpeed={0.5}
        minDistance={9} maxDistance={18} minPolarAngle={0.5} maxPolarAngle={1.15} enableDamping />
...
export default function SchoolTownMap3D({ activeId, onActiveChange, fallback = null }) {
  ...
  return (
    <WebGLBoundary fallback={fallback}>
      <div style={{ position: "relative", width: "100%", height: isMobile ? "clamp(300px, 52vh, 420px)" : "clamp(420px, 60vh, 640px)" }}>
        <Canvas shadows={!isMobile} dpr={isMobile ? [1, 1.5] : [1, 2]}
          camera={{ position: isMobile ? [10, 9, 12] : [9, 8, 11], fov: isMobile ? 40 : 34 }}
          gl={{ antialias: !isMobile, powerPreference: "high-performance" }}
          style={{ touchAction: isMobile ? "pan-y" : "none" }}>
          <color attach="background" args={["#fdf3ea"]} />
          <Suspense fallback={null}>
            <Scene setActive={setActive} activeId={activeId} hoveredId={hoveredId} setHovered={setHovered} reduced={reduced} isMobile={isMobile} />
          </Suspense>
        </Canvas>
