...
const STYLES = `
  ...
  @keyframes rlp-pulseBtn    { 0%,100%{ transform:scale(1) } 50%{ transform:scale(1.05) } }
  ...
  @keyframes rlp-drift       { 0%{ transform:translateX(-10vw) } 100%{ transform:translateX(120vw) } }
  ...
  .rlp-join-btn,.rlp-form-btn { transition:transform .2s ease,box-shadow .2s ease;cursor:pointer; }
  .rlp-join-btn:hover,.rlp-form-btn:hover { transform:scale(1.08) rotate(-1deg);box-shadow:0 10px 22px rgba(68,68,65,.4); }
  .rlp-join-btn:active,.rlp-form-btn:active { transform:scale(.93); }
...
export default function ReferralPage() {
  ...
  const handleJoin = () => {
    ...
    const s=screen2Ref.current, b=joinBtnRef.current;
      ...
      <div className="rlp-root">
        ...
        {screen === 'scratch' && (
          ...
          >
            ...
            <span style={{position:'absolute',top:50,   left:-30,fontSize:22,opacity:.5, animation:'rlp-drift 24s linear infinite',   pointerEvents:'none'}}>☁️</span>
            <span style={{position:'absolute',bottom:70,left:-30,fontSize:16,opacity:.4, animation:'rlp-drift 30s linear infinite 6s', pointerEvents:'none'}}>☁️</span>

            <div style={{position:'relative',maxWidth:420,margin:'0 auto'}}>
              {/* emoji header */}
              <div style={{display:'flex',justifyContent:'center',alignItems:'center',gap:10,marginBottom:16,fontSize:30,lineHeight:1}}>
                {(['🎁','⭐','🌈','⭐','🎁'] as const).map((em,i)=>(
                  <span key={i} className="rlp-dodge" onMouseEnter={dodgeEnter} onMouseLeave={dodgeLeave}
                    style={{display:'inline-block',fontSize:i===2?36:undefined,
                      animation:`rlp-${i===2?'floatySlow':'floaty'} ${i===2?3.4:2.4+i*.2}s ease-in-out infinite ${i*.2}s`}}>
                    {em}
                  </span>
                ))}
              </div>
        ...
        {screen === 'reveal' && (
          ...
          >
            ...
            <span style={{position:'absolute',top:90, left:-30,fontSize:20,opacity:.5, animation:'rlp-drift 20s linear infinite',    pointerEvents:'none'}}>☁️</span>
            <span style={{position:'absolute',top:220,left:-30,fontSize:15,opacity:.4, animation:'rlp-drift 26s linear infinite 5s',  pointerEvents:'none'}}>☁️</span>
            <span style={{position:'absolute',bottom:30,left:-30,fontSize:16,opacity:.45,animation:'rlp-drift 23s linear infinite 10s',pointerEvents:'none'}}>✨</span>
            ...
            <div style={{position:'relative',zIndex:2,maxWidth:440,margin:'0 auto'}}>
              ...
              <div className="rlp-shine"
                ...
                <span style={{position:'absolute',top:10,left:14, fontSize:14,animation:'rlp-twinkle 2s ease-in-out infinite',zIndex:1}}>✨</span>
                <span style={{position:'absolute',top:14,right:16,fontSize:12,animation:'rlp-twinkle 2.4s ease-in-out infinite .4s',zIndex:1}}>✨</span>
              ...
              <div style={{marginTop:18,animation:'rlp-slideUp .5s ease-out .56s both'}}>
                <button
                  ref={joinBtnRef}
                  onClick={handleJoin}
                  className="rlp-join-btn rlp-shine"
                  style={{background:'#444441',color:'#fff',border:'none',borderRadius:24,
                    padding:'12px 40px',fontSize:14,fontWeight:500,letterSpacing:'.5px',
                    animation:'rlp-pulseBtn 1.8s ease-in-out infinite',position:'relative',overflow:'hidden'}}
                >
                  👀 JOIN NOW
                </button>
                <div style={{fontSize:12,color:'#5F5E5A',marginTop:12}}>
                  Contact your nearest Rainbow school office
                </div>

                {joinExpanded && (
                  <div style={{marginTop:20,display:'flex',flexDirection:'column',alignItems:'center',gap:8,
                    animation:'rlp-slideUp .4s ease-out forwards'}}>
                    {/* Refer button */}
                    <a
                      ref={referBtnRef}
                    ...
                    <div style={{marginTop:14,fontSize:13,fontWeight:600,color:'#26215C',
                      ...
                      animation:'rlp-bob 2.2s ease-in-out infinite'}}>
                    ...
                    <div style={{display:'flex',gap:12,flexWrap:'wrap',justifyContent:'center',marginTop:2}}>
                      ...
                      >
                        <span style={{fontSize:26,display:'inline-block',animation:'rlp-floaty 2.4s ease-in-out infinite'}}>🎈</span>
                      ...
                      <button
                        ...
                        style={{width:130,background:'linear-gradient(160deg,#EEEDFE,#DCD8FB)',border:'none',
                          ...
                          alignItems:'center',gap:4}}
                      >
                        <span style={{fontSize:26,display:'inline-block',animation:'rlp-floaty 2.4s ease-in-out infinite .3s'}}>🎓</span>
