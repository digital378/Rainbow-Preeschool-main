/**
 * /dummy — Homepage Redesign Prototype v3 (Immersive + Premium + Animated)
 ...
 * CSS-only decorative elements + minimal Intersection Observer.
 * No Lottie. No GSAP. No framer-motion.
 */
...
/* ═══════════════════════════════════════════════════════════════════════════════
   SCOPED STYLES — keyframes + animation utilities
═══════════════════════════════════════════════════════════════════════════════ */
const STYLES = `
  /* floating orbs */
...
/** One shared IntersectionObserver that triggers du-vis on all .du-fade elements */
...
/** Counts from 0 to `target` when the span scrolls into view */
  ...
  return (
    <div className="relative -mt-px overflow-hidden pointer-events-none" style={{ height: 64 }}>
      <svg viewBox="0 0 1440 64" className="absolute bottom-0 w-full" preserveAspectRatio="none" style={{ height: 64 }}>
        <path d="M0,20 C200,60 500,0 720,32 C940,64 1200,8 1440,36 L1440,64 L0,64 Z" fill={fill} />
      </svg>
    </div>
  );
}
...
export default function Dummy() {
  ...
  /* attach scroll-reveal observer after first render */
  useFadeObserver();
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 sm:gap-4">
            {quickLinks.map(({ href, label, Icon, color }, i) => (
              <a key={href} href={href}
                className="du-fade group flex flex-col items-center gap-2 p-3 sm:p-4 rounded-2xl text-center min-h-[88px] justify-center transition-all duration-200 hover:-translate-y-1.5"
                style={{
        ...
        <div className="du-fade relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col md:flex-row md:items-center gap-4 md:gap-6">
            ...
            <div className="flex flex-col md:flex-row items-stretch gap-3 flex-1">
              <input placeholder="Your Name" className="flex-1 h-11 rounded-xl border border-input bg-white/90 px-4 text-sm shadow-[inset_0_1px_3px_rgba(0,0,0,0.04)] focus:outline-none focus:ring-2 focus:ring-primary/20" />
              <input placeholder="Phone Number" type="tel" className="flex-1 h-11 rounded-xl border border-input bg-white/90 px-4 text-sm shadow-[inset_0_1px_3px_rgba(0,0,0,0.04)] focus:outline-none focus:ring-2 focus:ring-primary/20" />
      ...
      <section className="relative py-20 md:py-32 overflow-hidden"
        ...
        <div className="absolute -top-32 -right-32 w-[520px] h-[520px] rounded-full border border-amber-200/22 pointer-events-none" />
        <div className="absolute -top-20 -right-20 w-[380px] h-[380px] rounded-full border border-amber-200/15 pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-14 lg:gap-20 items-center">
            ...
            <div className="du-fade">
              ...
              <p className="text-muted-foreground leading-[1.72] mb-4">
              ...
              <p className="text-muted-foreground leading-[1.72] mb-10">
                All six centres are in Thane West. Whether you are in Manpada, Kalwa, Dhokali, or Kasarvadavali, a Rainbow Preschool centre is always close to home.
              </p>
              <a href="/about"
            ...
            {/* Stats 2×2 — animated counters */}
            <div className="grid grid-cols-2 gap-4 sm:gap-5">
              {stats.map(({ Icon, label, accent, bg, border, target, format }, i) => (
                <div key={label}
                  className={cn(
                    "du-fade group relative rounded-2xl overflow-hidden p-5 sm:p-6",
                    "hover:-translate-y-2 transition-all duration-[300ms]",
                  ...
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 16px 40px ${accent}22,0 6px 12px rgba(0,0,0,0.08)`; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 24px rgba(0,0,0,0.07),0 1px 4px rgba(0,0,0,0.05)"; }}>
                  {/* Colored top accent bar */}
                  <div className="absolute top-0 left-0 right-0 h-[3px] rounded-t-2xl"
                    style={{ background: `linear-gradient(90deg,${accent},${accent}88)` }} />
                  {/* Soft corner glow */}
                  <div className="absolute -top-6 -right-6 w-24 h-24 rounded-full opacity-40 group-hover:opacity-65 transition-opacity"
      ...
      <section className="relative py-20 md:py-28 bg-white overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 lg:gap-7">
            {programmes.filter(p => !["kids-activity-club","summer-camp"].includes(p.id)).map((p, i) => {
              const ac = badgeColors[p.id] || "#ef4444";
              return (
                <div key={p.id}
                  className="du-fade prog-card group h-full flex flex-col cursor-pointer overflow-hidden rounded-2xl bg-white border border-gray-100 transition-all duration-[300ms] hover:-translate-y-2"
                  style={{
                    boxShadow: "0 4px 24px rgba(0,0,0,0.07),0 1px 4px rgba(0,0,0,0.04)",
                    transitionDelay: `${i * 80}ms`,
                  }}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 24px 52px rgba(0,0,0,0.12),0 8px 18px ${ac}24`; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 24px rgba(0,0,0,0.07),0 1px 4px rgba(0,0,0,0.04)"; }}>
                  ...
                  <div className="flex flex-col flex-1 p-5 sm:p-6 gap-2.5">
                    <h3 className="font-heading font-semibold text-[15px] text-foreground group-hover:text-primary transition-colors duration-150"
                      style={{ letterSpacing: "-0.01em" }}>
                      {p.name}
                    </h3>
                    <p className="flex-1 text-sm text-muted-foreground leading-relaxed line-clamp-3">{p.description}</p>
                    {/* Animated underline link */}
                    <div className="mt-2 relative inline-flex items-center gap-1.5 text-sm font-semibold" style={{ color: ac }}>
                      <span className="relative after:absolute after:bottom-0 after:left-0 after:w-0 after:h-[1.5px] after:bg-current after:transition-all after:duration-250 group-hover:after:w-full">
                        Learn More
                      </span>
                      <ArrowRight className="w-4 h-4 group-hover:translate-x-1.5 transition-transform duration-150" />
                    </div>
      ...
      {/* Wave to Why Choose Us */}
      <WaveDown fill="hsl(var(--card))" />
      ...
      <section className="relative py-20 md:py-28 overflow-hidden"
        ...
        <div className="absolute -bottom-40 -right-40 w-[580px] h-[580px] rounded-full border border-gray-200/50 pointer-events-none" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-5 md:gap-6">
            ...
            {/* Safety hero tile (2×2) */}
            <div className={cn(
              "du-fade relative md:col-span-2 md:row-span-2 rounded-2xl border overflow-hidden flex flex-col justify-between p-8 md:p-10 min-h-[300px] md:min-h-[480px]",
              ...
              onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 24px 52px rgba(239,68,68,0.16),0 8px 20px rgba(0,0,0,0.08)`; }}
              onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 24px rgba(0,0,0,0.07),0 1px 4px rgba(0,0,0,0.04)"; }}>

              {/* Decorative orbs inside tile */}
              ...
              <div className="relative">
                ...
                <p className="text-muted-foreground leading-[1.72] text-[16px] max-w-md">
                  {heroF.description}
                </p>
              </div>
              {heroF.highlight && (
                <div className="relative mt-8 pt-6 border-t border-red-200/55">
                  <p className="text-sm font-semibold text-red-600 leading-relaxed">{heroF.highlight}</p>
                </div>
              )}
            </div>

            {/* Small bento tiles — staggered */}
            {restF.map((f, i) => (
              <div key={i}
                className={cn(
                  "du-fade group relative rounded-2xl border p-6 flex flex-col gap-4 overflow-hidden",
                ...
                onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = `0 14px 36px ${f.accent}22,0 4px 10px rgba(0,0,0,0.06)`; }}
                ...
                <div>
                  <h3 className="font-heading font-semibold text-[15px] text-foreground mb-2" style={{ letterSpacing: "-0.01em" }}>{f.title}</h3>
      ...
      <section className="relative py-20 md:py-32 overflow-hidden"
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none overflow-hidden select-none" aria-hidden>
          <span className="font-serif leading-none text-amber-200/15"
        ...
        <Orb cls="d-float-b w-48 h-48 bottom-[12%] left-[8%] opacity-50"
          style={{ background: "radial-gradient(circle,rgba(220,38,38,0.10) 0%,transparent 62%)", filter: "blur(22px)" }} />
        <StarDot cls="d-tw1 text-amber-400/60 top-[14%] left-[32%] w-4 h-4" />
        <StarDot cls="d-tw2 text-amber-300/50 bottom-[24%] right-[18%] w-3 h-3" />
        <StarDot cls="d-tw3 text-red-300/40 top-[62%] left-[12%] w-3 h-3" />
        ...
        <div className="relative max-w-7xl mx-auto px-5 sm:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {testimonials.map((t, i) => {
              ...
              return (
                <div key={t.id}
                  className="du-fade group h-full flex flex-col rounded-2xl bg-white border border-amber-100/60 overflow-hidden transition-all duration-[280ms] hover:-translate-y-2"
                  style={{
                    boxShadow: "0 4px 20px rgba(0,0,0,0.06),0 1px 4px rgba(0,0,0,0.04)",
                    transitionDelay: `${i * 70}ms`,
                  }}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 20px 44px rgba(251,191,36,0.16),0 8px 16px rgba(0,0,0,0.07)"; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLDivElement).style.boxShadow = "0 4px 20px rgba(0,0,0,0.06),0 1px 4px rgba(0,0,0,0.04)"; }}>
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="absolute inset-0 bg-[radial-gradient(ellipse_at_top_right,rgba(251,191,36,0.24)_0%,transparent_55%)]" />
        ...
        <Orb cls="d-float-a w-[380px] h-[380px] -top-24 -right-16 opacity-50"
        ...
        <div className="du-fade relative z-10 py-20 md:py-28 max-w-3xl mx-auto px-5 sm:px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-3 sm:gap-4">
            <a href="/contact"
              className="group inline-flex items-center justify-center gap-2 rounded-full px-10 font-semibold bg-white text-red-700 hover:bg-white/94 transition-all duration-200 hover:-translate-y-1 active:scale-95"
              style={{ height: 58, boxShadow: "0 8px 32px rgba(0,0,0,0.20),0 2px 8px rgba(0,0,0,0.12),inset 0 1px 0 rgba(255,255,255,.8)" }}>
              Request a Callback
